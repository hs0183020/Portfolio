"""
ETL Script for Loading Data into SQL Database

This script extracts data from various sources, performs necessary transformations,
and loads it into an SQL database.
...
if __name__ == '__main__':
    ...
    A_index = column_list.index('cov_com')
    B_index = column_list.index('contact_information_complete')
    df_covid_CS = df_covid.loc[:, list(df_covid.columns[0:5]) + list(df_covid.columns[A_index:B_index+1])]
    ...
    A_index = column_list.index('cov_verbal_consent')
    B_index = column_list.index('demographic_and_medical_history_complete')
    df_covid_DM = df_covid.loc[:, list(df_covid.columns[0:5]) + list(df_covid.columns[A_index:B_index+1])]
    ...
    #---->NOTE!-------------used df_drs from another piece of code. Please import SAS DRS dataset before running
